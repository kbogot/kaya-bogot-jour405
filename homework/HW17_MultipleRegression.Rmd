...
Some of the variables that seem to be correlated to voter turnout are unemployment and race. These values are also the most obviously linear, with lower unemployment leading to higher voter turnout, and a higher white population leading to higher turnout as well.
Education and income seemed to be less correlated to voter turnout.
It's also interesting and worth mentioning that there seems to be an obvious correlation between the two independent variable, income and bachelors-higher. 
...
This model is useful in predicting turnout.
Based on the coefficients and p-values, we can conclude that the independent variables that are most significant are income, median age, and Pct white.
Additionally, the residual standard error of 0.0217 tells us that the models predictions are off by only about +/- 2.2%. Based on the multiple and adjusted r-squared values, we can also conclude that the model accurately explains over 80% of the variation in turnout.
Finally, based on the f-statistic and p-value, we can see that our model is isueful in predicting the outcome of our data and that it is statistically significant.
...
This comparison shows that the full model performs the best, explaining far more of the variation in turnout than any of the added models. All the other models have noticeably lower adjusted R-squared values, meaning that removing variables makes the model less accurate. Even though some independent variables in the full model aren’t statistically significant, they still contribute to the pigger pisture of the full model. No alternative model is better than the full model.
...
# Model 5
model5 <- lm(turnout ~ pct_unemployed, data = md_turnout)
summary(model4)
...
Using this analysis, we could examine which factors are most significant in determining voter turnout in an election. It could be interesting to examine the change in this data over the years and see if efforts to address the deficits has made any difference.
